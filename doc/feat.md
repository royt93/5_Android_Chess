...
4. ✅ **Lưu & tiếp tục ván (THƯ VIỆN ĐA-SLOT)** — `GameSaveManager` thuần (snapshot kiểu FEN: từng quân loại+màu+đã-đi + lượt + en passant + đồng hồ 50-nước + quân ăn + đồng hồ cờ; KHÔNG reflection, an toàn R8). `Chess.captureSaveState()/loadSaveState()` chụp/dựng lại bàn (tái dùng engine: castling/2-ô pawn rights đúng qua cờ hasMoved/firstMove).
   - **Đa-slot (≤100, file-per-slot `filesDir/saves/<sessionId>.sav`, LRU xoá cũ nhất)**: auto-save theo **session** — ván giữ 1 `sessionId` từ lúc bắt đầu → onPause ghi đè đúng slot, xoá khi kết thúc/restart/chơi-lại. `moveCount` tích luỹ qua resume (`resumedBaseMoveCount + getMoveCount`).
   - **Màn `SavedGamesActivity`** (BaseActivity, edge-to-edge `UIUtils`, background lung linh: `bg_game_gradient` + art mờ + corner glow pulse): list ScrollView (không thêm dep RecyclerView) sort mới→cũ, mỗi row = **`MiniBoardView`** (vẽ glyph unicode từ snapshot, nhẹ RAM) + mode (1v1 / Vs AI·độ khó) + N nước + bên đi + đồng hồ + thời gian tương đối + nút xoá (confirm `showBasicDialog`). Tap row → resume (`RESUME_SESSION`).
   - **MainActivity**: nút "SAVED GAMES (N)" all-caps (`textAllCaps`) mở library; BỎ dialog cold-start "Tiếp tục ván?".
   - `ChessClock.restore` khôi phục giờ 2 bên. String 15 locale. **15 unit test** (`GameSaveManagerTest` 6 + `GameSaveRoundTripTest` 2 + `GameSaveSlotTest` 7: CRUD/sort/upsert/LRU-evict/empty/null). **Verify Pixel:** menu badge (2), library 2 row thumbnail đúng thế (e4e5 / Nf3), sort, xoá-confirm→còn 1, resume→đúng thế + đồng hồ. *(v1 giới hạn: không lưu undo-stack & threefold.)*
...
5. ✅ **Câu đố cờ** (Puzzles mate-in-1) — `FenParser` thuần (FEN → `GameSaveManager.SavedGame`, suy quyền nhập thành/đẩy-2-ô từ FEN) → **tái dùng `Chess.loadSaveState`** dựng thế. `Puzzle` + `PuzzleRepository` (5 câu nhúng: hậu/xe back-rank, K+Q, smothered mã, 2 xe). **ChessBoardActivity puzzle-mode** (intent `PUZZLE_INDEX`): tắt AI/đồng hồ/autosave, ẩn undo/restart/hint/moves; nước SAI → tự undo + "thử lại"; chiếu hết → dialog **"Solved!"** (Next/Puzzles). Màn `PuzzlesActivity` list (glow nền) + nút menu **PUZZLES**. String 15 locale.
   - **Test 3 tầng**: **6 unit `FenParserTest`** (parse toạ độ/placement/castling/ep/FEN sai) + **3 integration `PuzzleRepositoryTest`** (dựng từng câu qua loadSaveState model-only, thực thi nước giải, **engine xác nhận CHIẾU HẾT** qua `RecordingChessBoardView` ⇒ mọi câu là mate-in-1 thật). Tổng **238 test JVM xanh**.
   - **Verify thiết bị**: menu→PUZZLES→list→Puzzle 1 load đúng thế (ẩn nút thừa)→giải **Rd1-d8#**→dialog **"Solved!"**. 0 crash.
   - 🔧 **Audit fix (Critical)**: cửa sổ chờ undo nước sai cho phép đi quân Đen (puzzle `isVsComputer=false` nên guard không chặn) → thêm cờ `Chess.inputLocked` chặn input; verify nước sai → revert + "try again".
   - 🟢 **Đào sâu (option 3)**: **mate-in-2** (model "solution line" tổng quát; nước Trắng đúng→app **tự đi nước Đen đáp** rồi chờ nước mate — verify device Qg7→auto Kb8→**Qb7#**) + **lưu tiến độ** (`PuzzleProgress` SharedPreferences; list hiện **X/N + ✅** câu đã giải, persist) + **7 câu** (6 mate-in-1 + 1 mate-in-2) + back-button puzzle thoát thẳng (không "Quit game?"/ad).
   - 🟢 **Test 3 tầng đủ (option 2)**: +`FenParserTest`(6)/`PuzzleRepositoryTest`(4, replay trọn line→engine xác nhận mate, **bắt được 1 mate-in-2 sai khi soạn**) + **Robolectric** `PuzzleProgressRobolectricTest`(3, persist/idempotent/null-safe) + **Espresso** `PuzzlesActivityEspressoTest`(2, PASS Pixel 7 Pro). **Tổng 242 JVM + 6 Espresso xanh.**
8. ✅ **Bộ quân cờ** (piece sets — LIGHTWEIGHT) — `PieceSet` (5 bộ: Classic/Gold/Neon/Royal/Ember) tint runtime qua `PieceRenderer` (PorterDuff **MULTIPLY** giữ shading + hiệu ứng 3D), **KHÔNG thêm asset PNG** → APK nhẹ. `PieceSetManager` (SharedPreferences) + `Chess.refreshPieceVisuals()`/`Chessman.refreshDrawable()` (lưu `pieceResId`) đổi màu quân **LIVE**. Nút **"♟ Pieces"** trong action row → picker glass (`DialogUtils.showChoiceDialog`). String 15 locale.
   - **Test**: `PieceSetTest`(4, định nghĩa/parse) + `PieceSetManagerRobolectricTest`(2, default Classic + persist). **Verify thiết bị:** chọn Neon → quân Trắng → cyan live, quân Đen sắc hồng nhẹ; persist. **assembleRelease (R8+lintVital) qua sạch.**
   - *Giới hạn:* MULTIPLY ⇒ quân Đen (PNG tối) đổi màu nhẹ; chủ yếu recolor quân Trắng (bên người chơi).

> 🎉 **WAVE C HOÀN TẤT** (#5 Câu đố · #6 Phân tích · #7 Thành tích · #8 Bộ quân cờ). Tổng **248 JVM + 6 Espresso test xanh**; assembleDebug + assembleRelease (R8 full-mode + lintVital) đều PASS. Mọi feature verify thiết bị thật.

> 🔍 **Release-readiness pass (deep review + polish):** 2 agent review độc lập (correctness + leak/lifecycle/R8) → vá: hint thiếu guard `inputLocked`, `cache.put(null)` crash (PieceRenderer), tint static `volatile`, BitmapDrawable dùng `applicationContext` (tránh ghim Resources Activity), nút Hint puzzle. **Cải thiện #8:** rim light theo sắc bộ quân → quân Đen cũng "ăn" theme (viền màu). **QA cứng:** full Espresso device — test mới (Puzzles/Achievements/ChessBoard/MainActivity-fixed) **8/8 PASS**; **LeakCanary 0 leak**; *(2 VIP Espresso fail là pre-existing, không liên quan; đã fix test MainActivity stale do dialog time-control)*. → **9.5/10, release-ready.**
